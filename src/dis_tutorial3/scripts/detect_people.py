...
class FaceDetector(Node):
    def __init__(self):
        ...
        self.face_plane_filtering_threshold = 0.9
        self.number_of_detections_threshold = 12
    ...
    def rgb_callback(self, msg):
        ...
        try:
            ...
            try:
                ...
                self.camera_to_world_tf = self.tf_buffer.lookup_transform(
                    'map', 'oakd_rgb_camera_optical_frame', rclpy.time.Time())
            except Exception as e:
                self.get_logger().warn(f"TF transform failed (latest): {e}")
                self.camera_to_world_tf = None
            ...
            for r in results:
                for bbox in r.boxes.xyxy.cpu().numpy():
                    ...
                    if not self.is_face_on_wall_plane(x1, y1, x2, y2):
                        self.get_logger().warn("❌ Rejected: bounding box does not lie on a planar surface.")
                        continue
                    ...
                    if normal is not None and centroid is not None:
                        if self.camera_to_world_tf:
                            normal = self.transform_normal_to_world(normal, self.camera_to_world_tf)

                        self.get_logger().info(f"✅ Face normal: {normal}, centroid: {centroid}")
                        self.add_to_group(centroid, normal)
    ...
    def transform_normal_to_world(self, normal, tf_msg):
    ...
    def add_to_group(self, new_point, normal, dist_thresh=0.1, angle_thresh_deg=15):
        angle_thresh_rad = np.radians(angle_thresh_deg)

        for group in self.face_groups:
            dist = np.linalg.norm(group['point'] - new_point)
            avg_normal = np.mean(group['normals'], axis=0)
            avg_normal /= np.linalg.norm(avg_normal) + 1e-6
            normal_norm = normal / (np.linalg.norm(normal) + 1e-6)
            angle = np.arccos(np.clip(np.dot(avg_normal, normal_norm), -1.0, 1.0))

            if dist < dist_thresh and angle < angle_thresh_rad:
                group['points'].append(new_point)
                group['normals'].append(normal)
                alpha = 0.3
                group['point'] = (1 - alpha) * group['point'] + alpha * new_point
                return
    ...
    def publish_new_faces(self):
        ...
        for i, group in enumerate(self.face_groups):
            ...
            self.get_logger().info(f"🧍 Published face at {avg_pos} with normal {avg_norm}")

    def destroy_node(self):
        self.get_logger().info("\n📋 Dumping all saved face groups:")
        for i, group in enumerate(self.face_groups):
            avg_pos = np.mean(group['points'], axis=0)
            avg_norm = np.mean(group['normals'], axis=0)
            self.get_logger().info(
                f"🔹 Group {i + 1}: count={len(group['points'])}, "
                f"avg_pos={np.round(avg_pos, 3)}, avg_norm={np.round(avg_norm, 3)}"
            )
